...
// ================= ACTIONS ====================
// Обработчик Ajax-запроса для загрузки постов
add_action('wp_ajax_load_brands', 'load_brands');
add_action('wp_ajax_nopriv_load_brands', 'load_brands');

function load_brands()
{
    //Создаем запрос для получения постов из категории
    $args = array(
        'post_type' => 'brands',
        'posts_per_page' => -1,
        'orderby'=>'id',
    ...
    if (!empty($my_posts) && is_array($my_posts)) {
        foreach ($my_posts as $my_post) {
            get_template_part('/inc/ajax/brands-ajax', 'item', array('item' => $my_post));
        }
    } else {
...
if( function_exists('GSE') ) {
    ...
    GSE()::add_translation('your-comment','Текст');
}
